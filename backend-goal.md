...
### 3.4 Services
- Each service (study room, printing, Wi-Fi, tutoring, catalogue, hall hire) has a
  description, live availability/status, and an actual booking/reservation/signup action
  with persisted state (not just a button that does nothing).
- Opening hours per service or per facility, including an "open now" computed flag.
...
- Each shop gets a unique, reserved **subdomain** (e.g. `myshop.malangenihub.co.za`) that
  resolves to that shop's own storefront view, isolated from other shops' data.
- Shop owners manage their own **products/services catalogue** (name, description, price,
  image, availability/stock) scoped strictly to their shop.
- Customers can browse a shop's storefront and place a basic **order/enquiry**
  (cart-free is acceptable for v1: simple order with items + quantities + status).
- Shops can optionally be surfaced in the main hub's Explore/Services listings so the
  community discovers them without visiting the subdomain directly.
- Role model must distinguish at least: community member, shop owner/staff, and platform
  admin — with authorization rules enforcing that a shop owner can only manage their own
  shop's data.
...
- **Authentication & authorization**: real login (password hashing, session/JWT), route
  ...
  owner can manage their shop's products/orders).
